...
local time = create_textbox(beautiful.font_bold .. " 66", "00 : 00", beautiful.green)
local add_timer = create_textbox(beautiful.font_bold .. " 16", " Add timer", beautiful.purple)
local set_timer_button = create_textbox(beautiful.font_bold .. " 16", "  󱎫 ", beautiful.purple)
...
local create_button     = function(icon)
	...
	button:connect_signal("mouse::enter", function()
		button_bg.bg = beautiful.bg_normal
	end)
	...
	button:connect_signal("button::press", function()
		button_bg.bg = beautiful.system_black_dark
	end)
...
local play_pause = create_textbox(beautiful.font_bold .. " 20", " ", beautiful.yellow)
local reset = create_textbox(beautiful.font_bold .. " 20", " ", beautiful.red)
local forward = create_textbox(beautiful.font_bold .. " 20", " ", beautiful.blue)
local backward = create_textbox(beautiful.font_bold .. " 20", " ", beautiful.blue)
	...
	awful.prompt.run {
		...
		exe_callback = function()
			...
			play_pause.markup = '<span color="' ..
					beautiful.yellow .. '" font="' .. beautiful.font_bold .. " 20" .. '">' .. " " .. '</span>'
		end,
		keypressed_callback = function(mod, key, cmd)
			if character_entered <= 4 then
				if key == "1" or key == "2" or key == "3" or key == "4" or key == "5" or key == "6" or key == "7" or key == "8" or key == "9" or key == "0" then
					...
					time.markup = '<span color="' ..
							beautiful.yellow .. '" font="' .. beautiful.font_bold .. " 66" .. '">' .. input_entered .. '</span>'
				end
			end
		end
	}
...
local timer_running = function()
	...
	if total_time >= 0 then
		...
		time.markup = '<span color="' ..
				beautiful.green .. '" font="' .. beautiful.font_bold .. ' 66' .. '">' .. final_input .. '</span>'
	end
	...
	if total_time == 1 then
		...
		(
			{
				title = '<span color="' ..
						beautiful.fg_color .. '" font="' .. beautiful.font_bold .. ' 14' .. '">󱎫 Timer Widget</span>',
				text = '<span color="' .. beautiful.fg_color .. '">The timer has ended! . Hope that was a productive time </span>',
				timeout = 5,
			}
		)
	end
...
set_timer_button:connect_signal("button::release", function()
	...
	input_entered = "-- : --"
	character_entered = 0
	time.markup = '<span color="' .. beautiful.yellow .. '" font="' .. beautiful.font_bold .. " 66" .. '">' .. "-- : --" .. '</span>'
	set_time()
end)
